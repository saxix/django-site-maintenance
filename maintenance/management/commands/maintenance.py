...
class Command(LabelCommand):
    option_list = (make_option('--force', action='store_true', dest='ignore_session', default=False,
    ...
    def handle_label(self, cmd, **options):
        ...
        if cmd == 'check':
            print "Status: %s - Active sessions: %s" % ( STATUS._labels[api.status()], api.get_active_users())
        elif cmd == 'on':
